...
<img src="{{ "/assets/img/content/pr-mespp/Banner.png" | absolute_url }}" alt="review-mespp" class="post-pic"/>
<br />
<br />
...
- Here we talk about the belief propagation at any time instant t using a motion model. We can see from the second term that its being evolved from the previous belief
- $\mathbf{M}$ represents a known motion model of the object. It is n✕n matrix, where $\mathbf{M}_{uv}$ denotes the probability that the object will move from u to v in time interval [t,t+1]. This has been referred to as the dispersion model $\mathbf{D}$ in the original paper by Hollinger
